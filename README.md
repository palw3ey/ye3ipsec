...
- Create a network with ipv4 and ipv6, named mynet46
```bash
...
- Customise the network settings to be able to use the VPN container,  
  Set these values when using docker create or docker run :
```bash
--cap-add NET_ADMIN --cap-add SYS_MODULE --cap-add SYS_ADMIN \  
  ...
  -p 500:500/udp -p 4500:4500/udp -p 50:50/udp -p 51:51/udp \  
...
- Activate the IKEv1 roadwarrior profile : Xauth PSK and Xauth RSA  
  They are disabled by default. Set these values when using docker create or docker run :
```bash
